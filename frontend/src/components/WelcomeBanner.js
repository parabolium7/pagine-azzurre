import React from 'react';

export default function WelcomeBanner(props) {
  return (
    <div className="flash welcome-banner">
        <h1 className="row center welcome">Benvenuti in "Pagineazzurre" piazza dove barattiamo e scambiamo con meno Euro e più VAL ☯</h1>
        <h2 className="row center welcome">"Pagineazzurre" favorisce ogni scambio di prodotti servizi e competenze con moneta speculativa: Euro, $, Cripto, ecc... Meglio se non speculativa: VAL, Crediti, Din, ecc...</h2>
    </div>
  );
}